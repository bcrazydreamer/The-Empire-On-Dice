...
function CheckEmpire(x,y,l,b,key_state,plr){
	...
	$('td').removeClass('active-left');
	$('td').removeClass('active-right');
	$('td').removeClass('active-top');
	$('td').removeClass('active-bottom');
	for(var i=x+1 ; i <= x+l ; i++){
		for(var k=x+1 ; k <= x+b ; k++){
			var inc = game_col*y+k;
			element = $('td[data-pos="'+inc+'"]');
			if(k==x+1){
				element.addClass('active-left');
			}
			if(k==x+b){
				element.addClass('active-right');
			}
			if(i==x+1){
				element.addClass('active-top');
			}
			if(i==x+l){
